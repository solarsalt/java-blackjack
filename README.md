...
- 승패계산
  - 플레이어 중 한명이라도 카드의 합이 21을 초과하는 경우, '버스트'라고 하며 딜러의 결과에 관계없이 플레이어가 패배한다.
  - 이 룰은 게임 구현사항에서 다루지 않음. 단순히 플레이어 점수 vs 딜러 점수를 비교하여 승무패를 결정하기로 함.
...
- [x] 딜러, 게임참여자에게 카드 배분하고, 배분한 결과를 화면에 노출
  ...
  - [x] 배분결과를 화면에 노출
- [x] 각 게임참여자에게 카드 더 받기(Hit) 단계를 진행한다.
  - [x] 게임참여자에게 카드 더 받기(Hit) 여부를 물어본다.
  - [x] y이면, 카드를 1개 배분하고, 보유카드를 화면에 노출하기
  - [x] n이면, 카드 더 받기 단계 종료
  - [x] 다음 게임 참여자에게 카드 더 받기 단계 반복
- [x] 모든 게임참여자가 카드 더 받기 단계를 종료한 뒤, 딜러가 16이하면 카드를 한장 더 받고, 아니면 종료
- [x] 딜러, 모든 게임 참여자의 보유카드의 점수를 계산
- [x] 딜러, 모든 게임 참여자의 보유카드와 계산결과를 노출
- [x] 최종 승패 계산하기 
- [x] 최종 승패 계산 결과 노출 